...
if check_tentacle_version("0.9.52", "indicator_keywords", "Octane-Default-Tentacles"):
    try:
        from .indicator_keywords import *
    except Exception as e:
        get_logger("TentacleLoader").error(
            f"Error when loading indicator_keywords: "
            f'{e.__class__.__name__}{f" ({e})" if f"{e}" else ""}. If this '
            f"error persists, try reinstalling your tentacles via "
            f'"python start.py tentacles --install --all".'
        )

if check_tentacle_version("0.9.52", "basic_tentacles", "Octane-Default-Tentacles"):
    try:
        from .basic_tentacles import *
    except Exception as e:
        get_logger("TentacleLoader").error(
            f"Error when loading basic_tentacles: "
            f'{e.__class__.__name__}{f" ({e})" if f"{e}" else ""}. If this '
            f"error persists, try reinstalling your tentacles via "
            f'"python start.py tentacles --install --all".'
        )

if check_tentacle_version("0.9.52", "RunAnalysis", "Octane-Default-Tentacles"):
    try:
...
if check_tentacle_version("0.9.52", "block_factory", "Octane-Default-Tentacles"):
    try:
        from .block_factory import *
    except Exception as e:
        get_logger("TentacleLoader").error(
            f"Error when loading block_factory: "
            f'{e.__class__.__name__}{f" ({e})" if f"{e}" else ""}. If this '
            f"error persists, try reinstalling your tentacles via "
            f'"python start.py tentacles --install --all".'
        )
